...
const {Roles,Users, Circle, Unaccepted} = require('../collections.js');
...
exports.getUser = functions.https.onCall(async(data,context)=>{
    let id = data.id;
    return Users.doc(id).get().then(async (u)=>{
        if(u.exists){
...
exports.checkUnacceptedMember = functions.https.onCall(async(data,context)=>{
    let {phNo} = data;
    return Unaccepted.doc(phNo).get().then((d)=>{
        if(d.exists){
            return {message: 'Unaccepted Member', member: d.data()}
        }
        else{
            return {message: 'Not Unaccepted Member'}
        }
    })
})
...
exports.updateAcceptance = functions.https.onCall(async(data,context)=>{
    let uid = context.auth.uid;
    let {phno,cid,timestamp} = data;
    return Circle.doc(cid).collection('Members').doc(phno).update({
        status: 'Accepted',
        timestamp : timestamp,
    })
})
